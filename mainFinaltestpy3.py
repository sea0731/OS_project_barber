...
class Barber( threading.Thread ):
    ...
    def run( self ):
        ...
        while 1:
            ...
            time.sleep(np.random.poisson(10, 1))
            self.enter.acquire()
            workingBarber[self.threadID] = 2
            self.enter.release()
...
class mainObject( threading.Thread ):             #main function of  barber-client problem
    ...
    def run( self ):
        ...
        sleeptime = np.random.poisson(float(Clientp), 10000)
	
        i = 0	
	
        while True:  #If user input "Y" terminate creating client
            time.sleep(sleeptime[i])    #randomly create client
            i += 1
...
class popupWindow(object):
    def __init__(self,master):
        ...
        self.l1=tk.Label(top, text="Input the number of Barber(1-3): ")
        self.l1.grid(row=0)
        self.e1=tk.Entry(top)
        self.e1.grid(row=0, column=1)
        self.l2=tk.Label(top, text="Input the number of Chair(1-5): ")
        self.l2.grid(row=1)
...
class mainwindow(tk.Frame):
    def __init__(self, master):
        ...
        self.button2 = tk.Button(self.canvas, text = "Start!", command = func)
	...
	# WorkingBarber here has to be syncronized
        ...
        while True:
            ...
            self.store = create(self.canvas, "2.png", 500, 362)
            #show if people come in
	    if whetherIn <= 3:
		if whetherIn == 1:		
		    whetherIn = 2
		    self.peopleIn = create(self.canvas, "arrow-s.png", 900, 300)
		if whetherIn == 2:                
		    whetherIn = 3
		if whetherIn == 3:
            ...
            #show if people come out
	    if whetherOut <= 3:
		if whetherOut == 1:		
		    whetherOut = 2
		    self.peopleOut = create(self.canvas, "arrow-s.png", 100, 300)
		if whetherOut == 2:                
		    whetherOut = 3
		if whetherOut == 3:
		...
		if ClientQueue.qsize() == 1:
            ...
            #show whether the barbor is working
	    if mBarber == 1:
		...
		for i in range (0, mBarber):
		    ...
		    if i==0 and workingBarber[i] == 2:
			self.arrowOut1 = create(self.canvas, "arrow.png", 500, 20)
			workingBarber[i] = 0
	    elif mBarber == 2:
		...
		for i in range (0, mBarber):
		    ...
		    if i==0 and workingBarber[i] == 2:
			self.arrowOut1 = create(self.canvas, "arrow.png", 375, 20)
			workingBarber[i] = 0
		    if i==1 and workingBarber[i] == 2:
			self.arrowOut2 = create(self.canvas, "arrow.png", 625, 20)
			workingBarber[i] = 0
	    elif mBarber == 3:
		...
		for i in range (0, mBarber):
		    ...
		    if i==0 and workingBarber[i] == 2:
			self.arrowOut1 = create(self.canvas, "arrow.png", 250, 20)
			workingBarber[i] = 0
		    if i==1 and workingBarber[i] == 2:
			self.arrowOut2 = create(self.canvas, "arrow.png", 500, 20)
			workingBarber[i] = 0
		    if i==2 and workingBarber[i] == 2:
			self.arrowOut3 = create(self.canvas, "arrow.png", 750, 20)
			workingBarber[i] = 0
